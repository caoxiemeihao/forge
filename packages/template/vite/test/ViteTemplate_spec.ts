...
describe('ViteTemplate', () => {
  ...
  context('template files are copied to project', () => {
    const expectedFiles = ['vite.main.config.js', 'vite.renderer.config.js', path.join('src', 'renderer.js'), path.join('src', 'preload.js')];
  ...
  it('should move and rewrite the main process file', async () => {
    ...
    expect(mainFile).to.match(/MAIN_WINDOW_VITE_ENTRY/);
    expect(mainFile).to.match(/MAIN_WINDOW_PRELOAD_VITE_ENTRY/);
  });
  ...
  it('should inject script into the HTML file', async () => {
    expect((await fs.readFile(path.join(dir, 'index.html'))).toString()).to.match(/src="\/renderer\.js"/);
  });
